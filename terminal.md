...
`git clone url`: is used to clone a repository given a specific link
...
`git commit -m "commit Message"`: is used to **commit** the files, and save their history. You can only use this command if you have staged the files using the `git add` command.
